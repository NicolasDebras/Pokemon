  ...
  async test(): Promise<Observable<Pokemon>> {
    ...
    while (!this.play) {
      ...
      if (this.Pokemon2_sub.stats[5].base_stat < this.Pokemon1_sub.stats[5].base_stat) {
        if (this.reciveDomagePokemon1(randomNumber_1) == 0)
          break;
        await new Promise(f => setTimeout(f, 1000));
        if (this.reciveDomagePokemon2(randomNumber_2) == 0)
          break;
        await new Promise(f => setTimeout(f, 1000));
      }
      else {
        if (this.reciveDomagePokemon2(randomNumber_1) == 0)
          break;
        await new Promise(f => setTimeout(f, 1000));
        if (this.reciveDomagePokemon1(randomNumber_2) == 0)
          break;
        await new Promise(f => setTimeout(f, 1000));
      }
    }
